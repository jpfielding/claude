...
Follow the visual-explainer skill workflow. Read the reference template at `./templates/slide-deck.html` and slide patterns in `./references/slides-*.md` files before generating. Also read `./references/css-layout.md` for shared patterns (Mermaid zoom controls, depth tiers, overflow protection) and `./references/libraries.md` for Mermaid theming, Chart.js, and font pairings.
...
**Aesthetic:** Pick a distinctive direction from the 4 slide presets in slides-responsive.md (Midnight Editorial, Warm Signal, Terminal Mono, Swiss Clean) or riff on the existing 8 aesthetic directions adapted for slides. Vary from previous decks. Commit to one direction and carry it through every slide.
